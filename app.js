...
async function connectToGitHub(options = {}) {
  ...
  try {
    ...
    const result = await loadStoreFromGitHub();
...
async function loadStoreFromGitHub() {
  const contentPath = `/repos/${state.repo.owner}/${state.repo.repo}/contents/${encodeURIComponentPath(state.repo.path)}?ref=${encodeURIComponent(state.repo.branch)}`;
...
function renderConnection() {
  elements.syncStatus.textContent = state.connected ? `GitHub: ${state.repo.owner}/${state.repo.repo}` : "Local demo";
  elements.syncStatus.classList.toggle("offline", !state.connected);
}
